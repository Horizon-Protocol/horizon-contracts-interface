...
describe('@synthetixio/js tests', () => {
	...
	beforeAll(() => {
		hznjs = horizon({ network: Network.Testnet });
	});
	...
	test('should include the supported networks', () => {
		expect(hznjs.networkToChainId[Network.Mainnet]).toBe(NetworkId.Mainnet.toString());
		expect(hznjs.networkToChainId[Network.Testnet]).toBe(NetworkId.Testnet.toString());
	});
	...
	test('should get the right synths data', () => {
		const validSynthIndex = findIndex(hznjs.synths, ({ name }) => name === 'zBNB');
